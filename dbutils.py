...
def insertHiveByImpyla(sql,  INCEPTOR_CONFIG_IMPYLA=DBConfig.INCEPTOR_CONFIG_IMPYLA):
    '''
    功能：使用Impyla向Inceptor插入数据
    参数：sql - sql语句
    返回：无
    '''
...
def selectHiveByImpyla(sql,  INCEPTOR_CONFIG_IMPYLA=DBConfig.INCEPTOR_CONFIG_IMPYLA):
    '''
    功能：使用Impyla查询Inceptor
    参数：sql - sql语句
    返回：数据集，DataFrame格式
    '''
...
def insertHiveByPyhive(sql, mode='cluster'):
    '''
    功能：使用pyhive向Inceptor插入数据
    参数：sql - sql语句
          model - 查询模式，可选参数为['cluster', 'local']，默认为'cluster'
    返回：无
    '''
    if mode == 'cluster':
...
def selectHiveByPyhive(sql, mode='cluster'):
    '''
    功能：使用pyhive查询Inceptor
    参数：sql - sql语句
          model - 查询模式，可选参数为['cluster', 'local']，默认为'cluster'
    返回：数据集，DataFrame格式
    '''
...
# def selectHiveByPyodbc(sql, INCEPTOR_CONFIG_PYODBC=DBConfig.INCEPTOR_CONFIG_PYODBC):
#     '''
